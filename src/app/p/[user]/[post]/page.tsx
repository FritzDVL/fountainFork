...
const post = async ({ params }: { params: { user: string; post: string } }) => {
  ...
  const forumThreadId = post?.metadata?.attributes?.find((attr: any) => attr.key === "forumThreadId")?.value;
  const forumThreadTitle = post?.metadata?.attributes?.find((attr: any) => attr.key === "forumThreadTitle")?.value;

  const hasEmbeddedTitle = contentJson ? (() => {
    try {
      const parsed = JSON.parse(contentJson);
      return Array.isArray(parsed) && parsed.some((node: any) => node.type === "title");
    } catch { return false; }
  })() : false;
          ...
          <div className="flex flex-col gap-4 items-center justify-center">
            ...
            {forumThreadId && (
              <span className="text-xl font-bold">
                Re: {forumThreadTitle || "Thread"}
              </span>
            )}
            {!forumThreadId && !hasEmbeddedTitle && title && (
              <span className="text-xl font-bold">{title}</span>
            )}
          </div>
          <Editor showToc value={contentJson} readOnly={true} />
          <div className="max-w-[72ch] mx-auto p-4 sm:p-8 md:px-16 flex flex-col gap-6 sm:gap-8">
